...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Order Update Status Error ${error}` });
  }
};
...
export const acceptOrder = async (req, res) => {
  try {
    ...
    if (alreadyAssign) {
      return res
        .status(400)
        .json({ message: "you Are already assign to another order" });
    }
...
export const getCurrentOrder = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Delivery Boy Get order  Error ${error}` });
  }
};
...
export const sendDeliveryOtp = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: `OTP send to ${order.user?.fullName}` });
  } catch (error) {
